...
def cleanup_expired_users():
    while True:
        with db_lock:
            ...
            cursor.execute('SELECT user_id FROM users WHERE expiration_time < ?', 
                         (now.strftime('%Y-%m-%d %H:%M:%S'),))
            expired_users = [row[0] for row in cursor.fetchall()]
            
            # Xóa khỏi database
            cursor.execute('DELETE FROM users WHERE expiration_time < ?', 
                         (now.strftime('%Y-%m-%d %H:%M:%S'),))
            
            # Xóa khỏi danh sách allowed_users trong bộ nhớ
...
def handle_link_replace(message):
    if message.from_user.id not in ADMIN_ID:
    ...
    if len(msg_parts) < 2:
        bot.reply_to(message, "Vui lòng cung cấp link chứa file live.txt\nVí dụ: /link https://raw.githubusercontent.com/.../live.txt")
        return
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            bot.reply_to(message, "Đã cập nhật live.txt từ liên kết thành công.")
        else:
            bot.reply_to(message, f"Lỗi khi tải file: {response.status_code}")
    except Exception as e:
...
def handle_add_user(message):
    ...
    if len(message.text.split()) == 3:
        ...
        if time_input[-1] == 'd':
            ...
            expiration_time = datetime.datetime.now() + datetime.timedelta(days=30 * months)
        elif time_input[-1] == 's':
...
def handle_add_token(message):
    ...
    with open("live.txt", "a") as file:
        for entry in msg[1:]:
            if '|' in entry:
                phone, token = entry.split("|", 1)
                file.write(f"{phone}|{token}\n")
                added_count += 1
            else:
                invalid_count += 1

    reply_msg = f"Đã thêm {added_count} dòng vào live.txt."
    if invalid_count > 0:
        reply_msg += f" Bỏ qua {invalid_count} dòng sai định dạng."
